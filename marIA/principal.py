...
def selecionar(individuos):
    torneio = 3
    selecionados = []
    for _ in range(len(individuos)):
        competidores = random.sample(individuos, torneio)
        vencedor = max(competidores, key=lambda n: n.fitness)
        selecionados.append(vencedor)
    return selecionados

def cruzamento(pai1, pai2):
    ponto_de_corte = random.randint(0, len(pai1.acoes))
    filho1 = Individuo()
    filho2 = Individuo()
    filho1.acoes = pai1.acoes[:ponto_de_corte] + pai2.acoes[ponto_de_corte:]
    filho2.acoes = pai2.acoes[:ponto_de_corte] + pai1.acoes[ponto_de_corte:]
...
def algoritmo_genetico(populacao, ambiente, geracoes=100):
    ...
    for geracao in range(geracoes):
        ...
        selecionadas = selecionar(populacao)
        descendentes = []
